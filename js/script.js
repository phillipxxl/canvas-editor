class Plugin_Collage_Maker_2 {
					...
					if (el.matches("[data-action]")) {
						...
						*/
						if (target == "info") {
							...
							this.meta_data.canvas_zoom =
								this.meta_data.canvas_zoom + 0.1;
							...
							["upload", "templates", "background"].includes(
								target,
							)
						) {
	...
	triggerToolzone(target, force_open = false) {
		...
		let has_zone = true;
		if (
			target == "autofill" || target == "clear" || target == "info" ||
			target == "save"
		) {
			has_zone = false;
		}
		if (has_zone) {
			...
			if (
				show_toolzone == 1 &&
				((!this.is_mobile && target != "text") || this.is_mobile)
			) {
				Helper.addClass(el, "active");
				Helper.removeClass(".tool-zone [data-zone].active", "active");
				document.querySelectorAll(
					'.tool-zone, .tool-zone [data-zone="' + target + '"]',
				).forEach(function (el) {
					Helper.addClass(el, "active");
				});
			}
		...
		fetch(this.api_url + "/get-template.php?all=1", {
			...
			Array.from(data).forEach((item, index) => {
				...
				if (item.visible == "1") {
					if (item.favorite == "1") {
						template_tags +=
							'<div><div class="tag" data-tag="favorite">Beliebt</div></div>';
					}
					/*
					if (item.text == "1") {
					...
					*/
					output += '<li data-id="' + item.ID + '" data-index-id="' +
						index + '" data-slots="' + item.slots +
						'" data-keywords="' + item.keywords +
						'" data-favorite="' + item.favorite + '">\
						<div class="thumb"><img loading="lazy" src="' +
						this.api_url + "/templates/thumbs/template-thumb-" +
						item.ID + '.jpg" alt="#' + item.ID + '" /></div>\
						<div class="tags">' + template_tags + "</div>\
					</li>";
				}
			});
			document.querySelector('[data-zone="templates"] > ul').innerHTML +=
				output;

			// add template click events
			Helper.addEvent(
				'.tool-zone [data-zone="templates"] li',
				"click",
				(e) => {
					var el = e.target;
					var template_index = el.getAttribute("data-index-id");
					Helper.removeClass(
						'.tool-zone [data-zone="templates"] li.active',
						"active",
					);
					Helper.addClass(el, "active");
					this.showTemplatePrompt(data[template_index]); // shows prompt if it is needed
					document.querySelector("#collage-designer").scrollIntoView({
						behavior: "smooth",
					});
				},
			);
	...
	showTemplatePrompt(template_data, callback = null) {
		...
		if (typeof template_data == "undefined") { // undefined = dyn
			...
			if (typeof meta_data.overlay_type != "undefined") {
				...
				if (prompt_number) {
					...
					meta_data.temp_overlay[0].file.includes("happy_number")
				) {
					...
					if (
						special_templates.some((num) =>
							meta_data.temp_overlay[0].file.includes(num)
						)
					) {
		...
		if (prompt_type != null) {
			if (prompt_type == "dyn") {
				prompt_content = '<form name="choose_template"><p>' +
					this.translate["prompt-text-headline"] +
					'</p><div class="wrap"><input name="prompt-text" placeholder="' +
					this.translate["prompt-text-placeholder"] +
					'"><button type="submit" class="cta">' +
					this.translate["prompt-button"] + "</button></div></form>";
			} else if (prompt_type == "classic_number") {
				prompt_content = '<form name="choose_template"><p>' +
					this.translate["prompt-number-headline"] +
					'</p><div class="wrap"><input type="text" inputmode="numeric" maxlength="2" name="prompt-number" placeholder="30"><button type="submit" class="cta">' +
					this.translate["prompt-button"] + "</button></div></form>";
			} else if (
				prompt_type == "happy_number" ||
				prompt_type == "happy_number_special"
			) {
				var prompt_content = '<form name="choose_template"><p>' +
					this.translate["prompt-number-headline"] +
					'</p><div class="wrap"><div class="select"><select name="prompt-select">';
				for (let i = 10; i <= 90; i = i + 10) {
					prompt_content += '<option value="' + i + '">' + i +
						"</option>";
					if (prompt_type == "happy_number_special" && i == 10) {
						prompt_content += '<option value="' + 18 + '">' + 18 +
							"</option>";
					}
				}
				prompt_content +=
					'</select></div><button type="submit" class="cta">' +
					this.translate["prompt-button"] + "</button></div></form>";
			}
			xxlPopup.create({
				data: {
				...
				ready: () => {
					document.querySelector('.xxl-overlay [name^="prompt-"]')
						.focus();
					Helper.addEvent(".xxl-overlay form", "submit", (e) => {
						e.preventDefault();
						if (prompt_type == "dyn") {
							let new_input =
								document.querySelector(".xxl-overlay input")
									.value;
							if (new_input != "") {
							...
							let new_number =
								document.querySelector(".xxl-overlay select")
									.value;
							meta_data.temp_overlay[0].file = meta_data
								.temp_overlay[0].file.replace(
									"_30",
									"_" + new_number,
								);
							console.log(meta_data);
	...
	setAutoFillLoading(state, text = null) {
		...
		if (text != null) {
			document.querySelector(
				"#collage-designer .editor > .autofill-loading",
	...
	autofill() {
		...
		for (const el of slots) {
			let img_pool = document.querySelectorAll(
				'.uploads .img-wrap[data-is-used="0"] img',
			);
	...
	resetCanvas() {
		...
		*/
		document.querySelectorAll(
			'[data-zone="background"] span.active, [data-zone="text"] span.active',
		).forEach((el) => {
			Helper.removeClass(el, "active");
		});
	}
	clearCanvas() {
		document.querySelectorAll('.uploads .img-wrap[data-is-used="1"]')
			.forEach((el) => {
	...
	saveCollageToDb(unset_borders = false) {
		...
		document.querySelector("#collage-designer .editor .canvas-wrap")
			.style["scale"] = this.meta_data.canvas_zoom;
		if (unset_borders) {
		...
		this.canvas.meta.viewportTransform = JSON.stringify(
	...
	updateCanvasMetaInfos(params) {
		...
		if (this.canvas.meta.width > this.canvas.meta.height) {
			this.canvas.meta.ratio = this.canvas.meta.width /
				this.canvas.meta.height;
		} else {
			this.canvas.meta.ratio = this.canvas.meta.height /
				this.canvas.meta.width;
		}
		...
		if (this.canvas.meta.ratio > 4.9) { // 51
			...
			this.canvas.meta.ratio > 2.4 && this.canvas.meta.ratio < 2.6
		) { // 52
			var render_w = 851;
			var render_h = 315;
			...
			this.canvas.meta.ratio > 1.4 && this.canvas.meta.ratio < 1.6
		) { // 32
			var render_w = 6000;
			var render_h = 4000;
		...
		*/
		...
		if (typeof data.mask !== "undefined") {
			...
			for (var mask of data.mask) {
				...
				var mask_img = this.api_url + "/templates/artworks-thumbs/" +
					mask.file;
		...
		if (typeof data.bg_motif !== "undefined") {
			...
			var bg_motif = this.api_url + "/templates/artworks-thumbs/" +
				data.bg_motif;
			...
			fabric.Image.fromURL(bg_motif, (myImg) => {
				...
				this.canvas.meta.bg_motif_image = this.api_url +
					"/templates/artworks-full/" + data.bg_motif;
		...
		if (this.canvas.meta.type == "default") {
			...
			if (vw <= 960) {
				...
				this.canvas.setDimensions({
					width: this.canvas.getWidth() / ratio,
					height: this.canvas.getHeight() / ratio,
				});
		...
		this.loadFont("Work Sans", () => {
			fetch("/collage-canvas-layouts-dyn.json", {
				...
				do {
					...
					ghost_canvas = new fabric.Canvas(ghost_canvas, {
						width: 1000,
						height: 1000,
					});
					...
					if (
						ghost_text_overlay.width * canvas_zoom_factor >
							canvas_puffer_width
					) {
				...
				this.canvas.zoomToPoint(
					new fabric.Point(canvas_w / 2, canvas_h / 2),
					canvas_zoom_factor,
				);
				...
				Array.from(this.canvas.meta.letter_data).forEach(
					(char, char_key) => {
						...
						if (last_text_pos_x == null) {
							var text_x = ((this.canvas.width - text_w) / 2) -
								puffer_x;
							var text_y = ((this.canvas.height - text_h) / 2) +
								puffer_y;
						} else {
							var text_x = last_text_pos_x + last_text_w +
								(char_spacing / 1.5);
							var text_y = ((this.canvas.height - text_h) / 2) +
								puffer_y;
						}
				...
				Array.from(temp_items).forEach((item, temp_key) => {
					...
					if (char.toLowerCase() in data) {
						Array.from(data[char.toLowerCase()]).forEach(
							(val, key) => {
								var scale_factor = initial_letter_size /
									item.height;
								var slot_w = val.width / scale_factor;
								var slot_h = val.height / scale_factor;
								var slot_x = (val.left / scale_factor) +
									item.left;
								var slot_y = (val.top / scale_factor) +
									item.top;
	...
	addCanvasInlineText(data = null) {
		if (this.canvas.meta.type == "dyn") {
			...
			var text_y = canvas_bounds.tl.y + empty_space_size +
				letter_bounds.h + (empty_space_size / 2) - (dyn_font_size / 2);
		...
		if (typeof data.text !== "undefined") {
			...
			this.canvas.meta.text_data.size = Math.ceil(
				this.canvas.meta.text_data.size,
			);
	...
	setUserTextOptionsDefaults(el = null) {
		...
		document.querySelector('.text-tools input[name="font-size"]').value =
			size;
		document.querySelector('.text-tools input[name="font-size"]')
			.setAttribute("value", size);
		document.querySelector(
			'.text-tools select[name="font-family"] option[value="' + family +
				'"]',
		).selected = true;
	...
	setTextOptionsColorPreview(target = this.active_text) {
		...
		document.querySelector('[data-zone="text"] .color-preview')
			.style["background-color"] = color;
	}
	refreshUserTextOptionsPosition() {
		let zoom = 1;
		if (
			document.querySelector(".canvas-wrap").style["scale"] != null &&
			document.querySelector(".canvas-wrap").style["scale"] != ""
		) {
			zoom = parseFloat(
				document.querySelector(".canvas-wrap").style["scale"],
			);
		...
		let editor_scroll_top =
			document.querySelector("#collage-designer .editor").scrollTop;
		let editor_scroll_left =
			document.querySelector("#collage-designer .editor").scrollLeft;
		let editor_position = document.querySelector(
			"#collage-designer .editor",
		).getBoundingClientRect();
		let container_position = document.querySelector(
			"#collage-designer .canvas-container",
		).getBoundingClientRect();
		let x = container_position.left - editor_position.x + text_box.left +
			((text_box.width / 2) - text_options_position.width / 2) +
			editor_scroll_left;
		let y = container_position.top - editor_position.y + text_box.top -
			text_options_position.height + editor_scroll_top;
	...
	addSlotUpload(obj) {
		obj.on("selected", () => {
			if (
				this.slot_upload === true &&
				!Helper.hasClass(document.querySelector("body"), "dragging")
			) {
				this.triggerToolzone("upload", true);
				this.canvas.meta.last_active_slot = this.canvas
					.getActiveObject();
			}
		});
	}
	initUploadPopup() {
		var content = document.querySelector("#upload-wrapper").content
			.cloneNode(true);
		var guided = false;
			...
			ready: () => {
				...
				if (guided) {
					...
					var canvas = new fabric.Canvas(
						document.querySelector(".view-0 canvas"),
					);
					canvas.loadFromJSON(canvasJSON, () => {
							...
							if (typeof val.meta !== "undefined") {
								if (val.meta.type == "slot-image") {
									...
									val.set({
										...
										fill: "#1e1e1e",
										textAlign:
											this.canvas.meta.text_data.align,
										fontFamily: "Open Sans",
										fontSize:
											this.canvas.meta.text_data.size,
										top: this.canvas.meta.text_data.top,
						...
						canvas.dispose();
						document.querySelector(".upload-wrapper .view-0 output")
							.innerHTML = '<img src="' + preview_canvas + '" />';
					});

					Helper.addEvent(
						".upload-wrapper .view-0 .js-back",
						"click",
						() => {
							window.history.back();
						},
					...
					Helper.addEvent(
						".upload-wrapper .view-0 button",
						"click",
						() => {
							Helper.removeClass(
								".upload-wrapper .view-0",
								"active",
							);
							Helper.addClass(
								".upload-wrapper .view-1",
								"active",
							);
						},
					);
				...
				document.querySelectorAll(".upload-wrapper .view-0 .dragzone")
					.forEach((el) => {
				...
				Helper.addEvent(".upload-wrapper .js-switch", "click", (e) => {
					...
					// add qr code
					document.querySelector(".upload-wrapper .qr-code")
						.innerHTML = '<img style="max-width: 200px;" src="' +
							this.api_url + "/gen-qr-code.php?sess_id=" +
							this.sess_id + '" alt="qr-code" />';

					// check every x seconds if images are there and download them
					var temp_interval = setInterval(() => {
						fetch(
							this.api_url + "/scan-for-files.php?sessid=" +
								this.sess_id,
							{
								method: "GET",
							},
	...
	async addImageToSlot(
		...
		document.querySelector(
			'.tool-zone .uploads .img-wrap[data-id="' + img_id + '"]',
		).setAttribute("data-is-used", 1);
		...
		if (target_obj.meta.type == "slot-image") {
			...
			let temp_obj = this.findCanvasObject({
				type: "slot",
			...
			temp_obj = this.findCanvasObject({
				type: "slot-image",
				slot_id: target_obj.meta.slot_id,
			});
	...
	imageActionZoom(slot_id, zoom_mode) {
		var obj =
			this.findCanvasObject({ type: "slot-image", slot_id: slot_id })[0];
		...
		if (zoom_mode == "in") {
			...
			new_scale_y = 1 - zoom_ratio;
			if (
				obj.scaleX * new_scale_x < obj.meta.initial_zoom_x ||
				obj.scaleY * new_scale_y < obj.meta.initial_zoom_y
			) {
		...
		if (slot_id == null && target_slot != null) {
			...
			if (obj.meta.slot_id == target_slot.meta.slot_id) {
				this.addImageToSlot(
					obj,
					document.querySelector(
						'.uploads .img-wrap[data-id="' + swap_img_id + '"] img',
					),
				);
				...
				this.addImageToSlot(
					val,
					document.querySelector(
						'.uploads .img-wrap[data-id="' + target_img_id +
							'"] img',
					),
				);
			}
	...
	imageActionRemove(params) {
		...
		this.removeSlotOptions();
		var slot_img_obj = this.findCanvasObject({
			type: "slot-image",
		...
		// restore default bg color
		var slot_obj =
			this.findCanvasObject({ type: "slot", slot_id: params.slot_id })[0];
		slot_obj.set({
		...
		if (counter == 0) {
			document.querySelector(
				'.uploads .img-wrap[data-id="' + img_id + '"]',
			).setAttribute("data-is-used", 0);
		}
	...
	imageActionEdit(slot_id) {
		const content = document.querySelector("#image-edit-wrap").content
			.cloneNode(true);
		const el =
			this.findCanvasObject({ type: "slot-image", slot_id: slot_id })[0];
		var edit_meta = {
		...
		const _updateBeforeAfter = function () {
			if (
				document.querySelector(
					'.image-edit-wrap [data-action="beforeafter"]',
		...
		const _updateClippingMask = (fire_event = false) => {
			if (fire_event) {
				edit_meta.canvas.fire("object:moving", {
					target: edit_meta.crop_area,
				});
			}
			if (edit_meta.bg_image_raw != null) {
				edit_meta.bg_image_raw.setPositionByOrigin(
					edit_meta.bg_image.getCenterPoint(),
					"center",
					"center",
				);
			...
			if (edit_meta.crop_area_mask != null) {
				edit_meta.crop_area_mask.setPositionByOrigin(
					edit_meta.crop_area.getCenterPoint(),
					"center",
					"center",
				);
		...
		const _checkImageQuality = () => {
			...
			document.querySelector(".xxl-overlay .cropper-wrap .quality")
				.setAttribute("data-quality", quality);
			document.querySelector(".xxl-overlay .cropper-wrap .quality")
				.setAttribute("title", this.translate["quality_" + quality]);
		};
		const _adjustBgImageAndCanvasRotation = () => {
			if (
				edit_meta.bg_image.angle == 90 ||
				edit_meta.bg_image.angle == 270
			) {
		...
		const _applyFilters = (obj, new_filters = null, init = false) => {
			...
			if (init == false) {
				for (
					const property of ["brightness", "contrast", "saturation"]
				) {
					var value;
					if (this.is_mobile) {
						value = parseFloat(
							document.querySelector(
								'.image-edit-wrap .settings > .mobile [data-target="' +
									property + '"] .filter-value',
							).gettAttribute("data-value"),
						);
					} else {
						var target = document.querySelector(
							'.image-edit-wrap .settings > .filter [name="' +
								property + '"]',
						);
			...
			if (obj.meta.type == "bg-image-raw") {
				const has_blend_filter = obj.filters.some((filter) =>
					filter instanceof fabric.filters.BlendColor
				);
				if (!has_blend_filter) {
					obj.filters.push(
			...
			if (
				typeof document.querySelector(".image-edit-wrap .settings") !==
					"undefined" &&
				document.querySelector(".image-edit-wrap .settings") != null &&
				init === true
			) {
				document.querySelector(
					'.image-edit-wrap .settings [name="brightness"]',
				).setAttribute("value", temp_filters.brightness * 100);
				document.querySelector(
					'.image-edit-wrap .settings [data-target="brightness"] .filter-value',
				).setAttribute("data-value", temp_filters.brightness * 100);
				document.querySelector(
					'.image-edit-wrap .settings [data-target="brightness"] .filter-value',
				).innerHTML = temp_filters.brightness * 100;
				Helper.triggerEvent(
					document.querySelector(
						'single-range-slider[name="brightness"]',
					).el_slider,
					"change",
				);
				document.querySelector(
					'.image-edit-wrap .settings [name="contrast"]',
				).setAttribute("value", temp_filters.contrast * 100);
				document.querySelector(
					'.image-edit-wrap .settings [data-target="contrast"] .filter-value',
				).setAttribute("data-value", temp_filters.contrast * 100);
				document.querySelector(
					'.image-edit-wrap .settings [data-target="contrast"] .filter-value',
				).innerHTML = temp_filters.contrast * 100;
				Helper.triggerEvent(
					document.querySelector(
						'single-range-slider[name="contrast"]',
					).el_slider,
					"change",
				);
				document.querySelector(
					'.image-edit-wrap .settings [name="saturation"]',
				).setAttribute("value", temp_filters.saturation * 100);
				document.querySelector(
					'.image-edit-wrap .settings [data-target="saturation"] .filter-value',
				).setAttribute("data-value", temp_filters.saturation * 100);
				document.querySelector(
					'.image-edit-wrap .settings [data-target="saturation"] .filter-value',
				).innerHTML = temp_filters.saturation * 100;
				Helper.triggerEvent(
					document.querySelector(
						'single-range-slider[name="saturation"]',
					).el_slider,
					"change",
				);
				if (obj.meta.filters.blackwhite == 1) {
					Helper.addClass(
						'.image-edit-wrap [data-action="blackwhite"]',
						"active",
					);
					document.querySelector(
						'.image-edit-wrap .settings [data-target="blackwhite"] toggle-switch',
				...
				edit_meta.crop_move_icon_box.set({
					left: edit_meta.crop_area.left +
						((edit_meta.crop_area.width *
							edit_meta.crop_area.scaleX) / 2) -
						(edit_meta.crop_move_icon_box.width / 2),
					top: edit_meta.crop_area.top +
						((edit_meta.crop_area.height *
							edit_meta.crop_area.scaleY) / 2) -
						(edit_meta.crop_move_icon_box.height / 2),
				});
		...
		const _setCropAreaCorners = () => {
			...
			["ml", "mr", "mt", "mb", "mtr"].forEach((control) =>
				edit_meta.crop_area.setControlVisible(control, false)
			);
		...
		xxlPopup.create({
			...
			ready: () => {
				...
				(async () => {
					const full_res_filename = document.querySelector(
						'.uploads .img-wrap[data-id="' + el.meta.img_id + '"]',
					).getAttribute("data-file");
					const full_res_src_url = this.api_url + "/user-files/" +
						this.sess_id + "/photos/" + full_res_filename;
					edit_meta.canvas_w = edit_meta.cropper_max_w;
					edit_meta.canvas_h = edit_meta.cropper_max_w /
						(el.width / el.height);
					if (el.height > el.width) {
						edit_meta.canvas_w = edit_meta.cropper_max_h /
							(el.height / el.width);
						edit_meta.canvas_h = edit_meta.cropper_max_h;
					}

					// init crop canvas
					document.querySelector(".image-edit-wrap .cropper")
						.innerHTML =
							'<span class="quality" data-quality="0"></span><canvas id="cropper-canvas"></canvas></div>';
					...
					edit_meta.bg_image = await this.fabricLoadImageFromUrl(
						full_res_src_url,
					);
					edit_meta.bg_image.set({
						originX: "top",
					...
					if (el.width > el.height) {
						edit_meta.bg_image.scaleToWidth(
							edit_meta.cropper_max_w,
						);
					} else {
						edit_meta.bg_image.scaleToHeight(
							edit_meta.cropper_max_h,
						);
					}
					edit_meta.bg_image.setCoords();
					...
					_applyFilters(
						edit_meta.bg_image,
						edit_meta.initial_filters,
					);
					_applyFilters(
						edit_meta.bg_image_raw,
						edit_meta.initial_filters,
					);
					...
					var clippath_aspect_ratio = el.clipPath.width /
						el.clipPath.height;
					var temp_x = (el.clipPath.left - slot_rect.left) *
						scale_factor;
					var temp_y = (el.clipPath.top - slot_rect.top) *
						scale_factor;
					var temp_w = edit_meta.canvas.width *
						(el.clipPath.width / slot_rect.width);
					...
					edit_meta.crop_move_icon_box = await this
						.fabricLoadImageFromUrl("images/button-icon-drag.svg");
					...
					edit_meta.crop_move_icon_box.set({
						...
						scalable: false,
						left: edit_meta.crop_area.left +
							(edit_meta.crop_area.width / 2) -
							(edit_meta.crop_move_icon_box.width / 2),
						top: edit_meta.crop_area.top +
							(edit_meta.crop_area.height / 2) -
							(edit_meta.crop_move_icon_box.height / 2),
					});
					...
					edit_meta.crop_aspect_ratio = edit_meta.crop_area.width /
						edit_meta.crop_area.height;
					...
					edit_meta.canvas.on("object:moving", (e) => {
						...
						const obj = e.target;
						const min_x = 0;
						const max_x = edit_meta.canvas.getWidth() -
							(obj.width * obj.scaleX);
						const min_y = 0;
						const max_y = edit_meta.canvas.getHeight() -
					...
					var temp_elements = [
						document.querySelector(
							'.image-edit-wrap [name="brightness"]',
						),
						document.querySelector(
							'.image-edit-wrap [name="contrast"]',
						),
						document.querySelector(
							'.image-edit-wrap [name="saturation"]',
						),
					];
					...
					Helper.addEvent(
						'.image-edit-wrap .tools-switcher [data-target="blackwhite"] toggle-switch',
						"change",
						(e) => {
							_handleBlackWhiteSwitch(e);
						},
					...
					Helper.addEvent(
						'.image-edit-wrap [data-action="mirror"]',
						"click",
						(e) => {
							edit_meta.bg_image.set(
								"flipX",
								!edit_meta.bg_image.flipX,
							);
					...
					Helper.addEvent(
						'.image-edit-wrap [data-action="maximize"]',
						"click",
						(e) => {
							let ratio = edit_meta.crop_area.width /
								edit_meta.crop_area.height;
					...
					Helper.addEvent(
						'.image-edit-wrap [data-action="rotate"]',
						"click",
						(e) => {
					...
					Helper.addEvent(
						...
						(e) => {
							if (!e.target.classList.contains("active")) {
								edit_meta.bg_image.temp_filters =
									edit_meta.bg_image.meta.filters;
								edit_meta.bg_image_raw.temp_filters =
									edit_meta.bg_image.meta.filters;
								e.target.querySelector("img").src =
									"images/filter-active-left.svg";
							} else {
								e.target.querySelector("img").src =
									"images/filter-active-right.svg";
							}
							Helper.toggleClass(e.target, "active");
							if (e.target.classList.contains("active")) {
								this.setImagePresetFilter(
									edit_meta.bg_image,
									null,
									false,
								);
					...
					Helper.addEvent(
						".image-edit-wrap .js-confirm-extra",
						"click",
						(e) => {
							Helper.toggleClass(
								".image-edit-wrap .confirm .hidden",
								"active",
							);
						},
					);
					Helper.addEvent(
						".image-edit-wrap .confirm .js-trigger",
						"click",
						(e) => {
							var bg_meta_filters = {};
							Object.entries(edit_meta.bg_image.meta.filters)
								.forEach((val, index) => {
									var name = val[0];
									var value = val[1];
					...
					Helper.addEvent(
						...
						(e) => {
							...
							console.log("CROP AREA", edit_meta.crop_area);
							let crop_area_w = edit_meta.crop_area.width *
								edit_meta.crop_area.scaleX;
							let crop_area_h = edit_meta.crop_area.height *
							...
							function convertRelativeToClipPath(
								object,
								left,
								top,
							) {
					...
					Helper.addEvent(
						...
						(e) => {
							...
							*/
								left: 0,
								top: 0,
								width: el.clipPath.width *
									edit_meta.bg_image_scale_w,
								height: el.clipPath.height *
									edit_meta.bg_image_scale_h,
								scaleX: 1,
								scaleY: 1,
							});
							edit_meta.crop_area.setCoords();
				...
				el.clone().then((img_clone) => {
					...
					var full_res_src = document.querySelector(
						'.uploads .img-wrap[data-id="' + img_clone.meta.img_id +
							'"] img',
					).getAttribute("src");
					...
					document.querySelector(".image-edit-wrap .cropper")
						.innerHTML =
							'<span class="quality" data-quality="0"></span><canvas id="cropper-canvas"></div>';
					edit_meta.canvas = new fabric.Canvas("cropper-canvas", {
					...
					fabric.Image.fromURL(img_clone, (img) => {
						...
						edit_meta.canvas.add(edit_meta.bg_image);
						if (
							edit_meta.bg_image.width > edit_meta.bg_image.height
						) {
							edit_meta.bg_image.scaleToWidth(
								edit_meta.cropper_max_w,
							);
						...
						edit_meta.canvas.setWidth(
							edit_meta.bg_image.width *
								edit_meta.bg_image_scale_w,
						);
						edit_meta.canvas.setHeight(
							edit_meta.bg_image.height *
								edit_meta.bg_image_scale_h,
						);
						edit_meta.canvas.requestRenderAll();
						...
						*/
						let temp_y = 0;
						let temp_x = 0;
						let temp_w = edit_meta.clip_path.width *
							edit_meta.bg_image_scale_w;
						let temp_h = edit_meta.clip_path.height *
							edit_meta.bg_image_scale_h;
						...
						if (this.canvas.meta.type == "dyn") {
							...
							temp_x = (pos_slot.left - pos_image.left) *
								edit_meta.bg_image_scale_w;
							temp_y = (pos_slot.top - pos_image.top) *
								edit_meta.bg_image_scale_h;
							let temp_w_percent = (100 / pos_slot.width) *
								pos_image.width;
							let temp_h_percent = (100 / pos_slot.height) *
								pos_image.height;
						}
						...
						aspectRatio = edit_meta.crop_area.width /
							edit_meta.crop_area.height;

						// add custom move icon in center of crop area
						fabric.Image.fromURL(
							"images/button-icon-drag.svg",
							(img) => {
								...
								edit_meta.crop_move_icon_box.set({
									...
									left: edit_meta.crop_area.left +
										(edit_meta.crop_area.width / 2) -
										(edit_meta.crop_move_icon_box.width /
											2),
									top: edit_meta.crop_area.top +
										(edit_meta.crop_area.height / 2) -
										(edit_meta.crop_move_icon_box.height /
											2),
								});

								edit_meta.canvas.add(
									edit_meta.crop_move_icon_box,
								);
								edit_meta.canvas.requestRenderAll();
							},
						);
						...
						edit_meta.initial_data = {
							width: edit_meta.clip_path.width *
								edit_meta.bg_image_scale_w,
							height: edit_meta.clip_path.height *
								edit_meta.bg_image_scale_h,
						};
					...
					*/
					...
					// add button events
					Helper.addEvent(
						'.image-edit-wrap .top-bar [data-action="prev"]',
						"click",
						(e) => {
					...
					Helper.addEvent(
						'.image-edit-wrap .top-bar [data-action="next"]',
						"click",
						(e) => {
							let new_slot = el.meta["slot_id"] + 1;
							let max_slots = this.countSlotImages() - 1;
					...
					Helper.addEvent(
						...
						(e) => {
							...
							if (target != "blackwhite" && target != null) {
								Helper.addClass(
									".image-edit-wrap .tools-wrap.filter",
									"active",
								);
								Helper.addClass(
									'.image-edit-wrap .tools-wrap.filter div[data-target="' +
										target + '"]',
									"active",
								);
							}
					...
					Helper.addEvent(
						".image-edit-wrap .js-confirm-filter",
						"click",
						(e) => {
							Helper.removeClass(
								".image-edit-wrap .tools-wrap.filter div[data-target].active",
								"active",
							);
							Helper.removeClass(
								".image-edit-wrap .tools-wrap.filter",
					...
					edit_meta.canvas.on("object:moving", (e) => {
						var obj = e.target;
						var min_x = 0;
						var max_x = edit_meta.canvas.getWidth() -
							(obj.width * obj.scaleX);
						var min_y = 0;
						var max_y = edit_meta.canvas.getHeight() -
							(obj.height * obj.scaleY);
	...
	addSlotOptions(target) {
		...
		<div>
			<div>
				<button type="button" onclick="collage_maker_2.imageActionZoom(` +
			slot.meta.slot_id +
			`, 'in')"><img src="images/add-circle.svg" /></button>
				<span class="divider"></span>
				<button type="button" onclick="collage_maker_2.imageActionZoom(` +
			slot.meta.slot_id +
			`, 'out')"><img src="images/remove-circle.svg" /></button>
			</div>
			<label>Zoom</label>
		</div>
		<button type="button" onclick="collage_maker_2.imageActionRotate(` +
			slot.meta.slot_id +
			`)"><img src="images/rotate-right.svg" /><label>Drehen</label></button>
		<button type="button" onclick="collage_maker_2.imageActionMirror(` +
			slot.meta.slot_id +
			`)"><img src="images/flip-horizontal.svg" /><label>Spiegeln</label></button>
		<button type="button" onclick="collage_maker_2.imageActionEdit(` +
			slot.meta.slot_id +
			`)"><img src="images/preference-horizontal.svg" /><label>Bild anpassen</label></button>
		<button type="button" onclick="collage_maker_2.imageActionSwap(` +
			slot.meta.slot_id +
			`)"><img src="images/image-swap.svg" /><label>Bild tauschen</label></button>
		<button type="button" onclick="collage_maker_2.imageActionRemove({slot_id:` +
			slot.meta.slot_id +
			`})"><img src="images/delete.svg" /><label>Löschen</label></button>
		`;
	...
	refreshSlotPositions() {
		if (this.is_mobile) {
			...
			var x = container_position.left - editor_position.x + slot_x +
				((slot_w / 2) - slot_options_position.width / 2) +
				editor_scroll_left;
			var y = container_position.top - editor_position.y + slot_y +
				slot_h + editor_scroll_top;
	...
	addThumbToUploadZone(data) {
		let temp_el =
			'<li><span class="tool js-delete" title="Löschen"></span><span class="tool js-preview-thumb" title="Vorschau"></span><span class="tool js-is-used" title="Wird verwendet"></span><span draggable="true" class="img-wrap" data-id="' +
			data.id + '" data-file="' + (data.file || "") +
			'" data-is-used="0"><img class="thumb" src="' + data.img_data +
			'" /></span></li>';
		document.querySelector(".uploads").innerHTML = temp_el +
	...
	updateUploadCounter() {
		const counter =
			document.querySelectorAll(".tool-zone .uploads li").length;
		let output = "";
		if (counter > 0) {
	...
	updateUploadView() { // todo
		Helper.addClass(
			'#collage-designer [data-zone="upload"] .view-2',
			"active",
		);
		Helper.removeClass(
			'#collage-designer [data-zone="upload"] .view-1',
			"active",
		);
	...
	deleteAllUploads() {
		...
		this.triggerWarning(content, () => {
			...
			Helper.addEvent(
				".xxl-overlay.warning .js-continue",
				"click",
				() => {
					this.db.canvas.clear();
					document.querySelectorAll(
						'.tool-zone [data-zone="upload"] li .img-wrap',
					).forEach((el) => {
	...
	zoomCanvas() {
		...
		let editor_bounding_box = document.querySelector(
			"#collage-designer .editor",
		).getBoundingClientRect();
		let canvas_bounding_box = document.querySelector(
			"#collage-designer .editor .canvas-wrap",
		).getBoundingClientRect();
		if (canvas_bounding_box.width > editor_bounding_box.width) {
			document.querySelector("#collage-designer .editor").scrollLeft =
				(canvas_bounding_box.width - editor_bounding_box.width) / 2;
		}

		var x = (editor_bounding_box.width / 2) -
			(canvas_bounding_box.width / 2);
		var y = (editor_bounding_box.height / 2) -
			(canvas_bounding_box.height / 2);
		if (canvas_bounding_box.width > editor_bounding_box.width) {
			x = 0;
		}
	...
	dev_orientation() {
		...
		var centerPoint = obj.translateToOriginPoint(
			obj.getCenterPoint(),
			"left",
			"top",
		);
	...
	async duplicateObject(object) {
		...
		const clonedObjects = await fabric.util.enlivenObjectsAsync([
			objectData,
		]);
	...
	initSessid() {
		if (window.localStorage.getItem("sess_id") !== null) {
			...
			const stringToHash =
				`${userAgent}-${language}-${currentTimestamp}-${rand}`;
			...
			function sha256(message) {
				...
				return crypto.subtle.digest("SHA-256", msgUint8).then(
					(hashBuffer) => {
	...
	copyCode() {
		var text = document.querySelector(".save-wrap tab-system").el_content
			.querySelector('form[name="link-code"] input').getAttribute(
				"value",
			);
